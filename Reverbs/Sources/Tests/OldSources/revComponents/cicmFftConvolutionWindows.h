...
#ifndef DEF_CICM_FFT_WINDOWS
#define DEF_CICM_FFT_WINDOWS

#include "CicmToolsWindows.h"
#include "cicmFft.h"
...
inline Cicm_Signal FftConvolution::process(Cicm_Signal anInput)
{
	...
	if(++m_ramp >= m_array_size)
	{
		...
		m_fft_instance->inverse(m_real_vector[0], m_input_complexes);
		...
		Cicm_signal_add(m_real_vector[0],  m_buffer, m_real_vector[0], m_array_size);
		m_ramp = 0;
	}
