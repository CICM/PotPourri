...
MbObjectManager::MbObjectManager(t_object* _mirageBoxObjectPointer, int _mirageboxUniqId)
{
    t_max_err err;
    m_mirageboxId = _mirageboxUniqId;
...
{
    // detach all objects
    for (std::map<int,MbObject>::iterator it = m_objects.begin(); it != m_objects.end(); ++it)
        object_detach_byptr(m_mbPointer, m_objects.at(it->first).getObjPtr());
    m_objects.clear();
}
...
t_object* MbObjectManager::getObjPtrByObjId(int _objectId)
{
    std::map<int,MbObject>::iterator it = m_objects.find(_objectId);
    return (it != m_objects.end()) ? m_objects.at(it->first).getObjPtr() : NULL;
}

int MbObjectManager::getObjIdByObjPtr(t_object* o)
{
    for (std::map<int,MbObject>::iterator it = m_objects.begin(); it != m_objects.end(); ++it)
        if (o == m_objects.at(it->first).getObjPtr()) return it->first;
    return -1;
}
...
void MbObjectManager::notify(t_symbol *s, t_symbol *msg, void *sender, void *data)
{
    ...
    if (sender == m_patcherview)
    {
        ...
        else if ( msg == gensym("startmoveboxes") )
        {
            ...
            for (std::map<t_object*,SelObject>::iterator it = m_uiSelectedObjects.begin(); it != m_uiSelectedObjects.end(); ++it)
                if (!object_isInTheBox(it->first)) object_attach_byptr_register(m_mbPointer, it->first, CLASS_BOX);
        }
        ...
        else if ( msg == gensym("endmoveboxes") )
        {
            ...
            for (std::map<t_object*,SelObject>::iterator it = m_uiSelectedObjects.begin(); it != m_uiSelectedObjects.end(); ++it)
            {
                ...
                if (!object_isInTheBox(it->first)) object_detach_byptr(m_mbPointer, it->first);
            }
        }
    }
    ...
    else
    {
        ...
        if (msg == gensym("attr_modified") && object_isInTheSelection(o))
        {
        ...
        {
            m_objects.at(getObjIdByObjPtr(o)).notify(s, msg, data);
...
{
    for (std::map<int,MbObject>::iterator it = m_objects.begin(); it != m_objects.end(); ++it)
        if (o == m_objects.at(it->first).getObjPtr()) return true;
    return false;
}

bool MbObjectManager::object_isInTheBox(int _objectId)
{
    return (m_objects.find(_objectId) != m_objects.end());
}
...
bool MbObjectManager::mb_removeboxIfInside(t_object* o)
{
    if (object_isInTheBox(o))
    {
        ...
        m_objects.erase(getObjIdByObjPtr(o));
...
bool MbObjectManager::mb_addboxIfInside(t_object* o)
{
    if (object_canBeAdded(o))
    {
        ...
        if ( jgraphics_rectcontainsrect(&m_mbDropAreaRect, &jr) && !object_isInTheBox(o) )
        {
            ...
            {
                mirageboxObjectId++;
                //post("mirageboxObjectId %i", mirageboxObjectId);
                
                m_objects.insert(std::pair<int,MbObject>(mirageboxObjectId, MbObject(m_mbPointer, o, mirageboxObjectId)));
...
{
    if (object_isInTheBox(o)) m_objects.at(getObjIdByObjPtr(o)).getValue(ac, av);
}

void MbObjectManager::getValue (int objId, long *ac, t_atom **av)
{
    if (object_isInTheBox(objId)) m_objects.at(objId).getValue(ac, av);
}

void MbObjectManager::getAttrValue (t_object* o, t_symbol* attrname, long *ac, t_atom **av)
{
    if (object_isInTheBox(o)) m_objects.at(getObjIdByObjPtr(o)).getAttrValue(attrname, ac, av);
}

void MbObjectManager::getAttrValue (int objId, t_symbol* attrname, long *ac, t_atom **av)
{
    if (object_isInTheBox(objId)) m_objects.at(objId).getAttrValue(attrname, ac, av);
}

t_symbol* MbObjectManager::getClassName(t_object* o)
{
    return object_isInTheBox(o) ? m_objects.at(getObjIdByObjPtr(o)).getClassName() : gensym("");
}

t_symbol* MbObjectManager::getClassName(int objId)
{
    return object_isInTheBox(objId) ? m_objects.at(objId).getClassName() : gensym("");
}

std::string MbObjectManager::getClassNameStr(t_object* o)
{
    std::string empty = "";
    return object_isInTheBox(o) ? m_objects.at(getObjIdByObjPtr(o)).getClassNameStr() : empty;
}

std::string MbObjectManager::getClassNameStr(int objId)
{
    std::string empty = "";
    return object_isInTheBox(objId) ? m_objects.at(objId).getClassNameStr() : empty;
}
...
void MbObjectManager::sendMessageToObject(t_object* o, long ac, t_atom *av)
{
    if (object_isInTheBox(o))
    {
        m_objects.at(getObjIdByObjPtr(o)).sendMessageToObject(ac, av);
    }
}

void MbObjectManager::sendMessageToObject(int objId, long ac, t_atom *av)
{
    if (object_isInTheBox(objId))
    {
        m_objects.at(objId).sendMessageToObject(ac, av);
    }
}

void MbObjectManager::sendMessageToObject(t_symbol* action, long ac, t_atom *av)
{
    if ( (ac >= 1) && (atom_gettype(av)) == A_LONG) {
        if (object_isInTheBox(atom_getlong(av))) {
            m_objects.at(atom_getlong(av)).sendMessageToObject(action, ac-1, av+1);
        }
    }
}
