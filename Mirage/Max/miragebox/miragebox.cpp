...
void miragebox_notifyApp(t_miragebox *x, t_symbol* notifyMsg, void *data)
{
    ...
    if (notifyMsg == gensym("newobject"))
    {
        ...
        if (ac && av)
        {
            long textAtomsSize = 0;
            char *textAtoms = NULL;
            atom_gettext(ac, av, &textAtomsSize, &textAtoms, 0);
            
            std::stringstream ss;
            ss << '/' << mirageboxSender << '/' << "newobject" << '/' << x->mb->getClassNameStr(objectPointer) << '/' << long(objectPointer);
            std::string stringOut = ss.str();
            
            t_atom out[1];
            atom_setsym(out, gensym(stringOut.c_str()));
            outlet_anything(x->f_out, gensym("list"), 1, out);
            sysmem_freeptr(textAtoms);
        }
    }
    ...
    else if (notifyMsg == gensym("freeobject"))
    {
        t_object *objectPointer = (t_object*)data;
        std::stringstream ss;
        ss << '/' << mirageboxSender << '/' << "freeobject" << '/' << long(objectPointer);
        std::string stringOut = ss.str();
        t_atom out[1];
        atom_setsym(out, gensym(stringOut.c_str()));
        outlet_anything(x->f_out, gensym("list"), 1, out);
    ...
    else if (notifyMsg == gensym("setvalue"))
    {
        ...
        if (ac && av)
        {
            ...
            atom_gettext(ac, av, &textAtomsSize, &textAtoms, 0);
            std::stringstream ss;
            ss << '/' << mirageboxSender << '/' << long(objectPointer) << '/' << "setvalue" << '/' << textAtoms;
            std::string stringOut = ss.str();
            t_atom out[1];
            atom_setsym(out, gensym(stringOut.c_str()));
            outlet_anything(x->f_out, gensym("list"), 1, out);
    ...
    else if (notifyMsg == gensym("setattr"))
    {
        ...
        t_object *objectPointer = (t_object*)atom_getobj(infos);
        t_symbol *attrname = atom_getsym(infos+1);
        std::stringstream ss;
        ss << '/' << mirageboxSender << '/' << long(objectPointer) << '/' << "setattr" << '/' << attrname->s_name;
        
        x->mb->getAttrValue(objectPointer, attrname, &ac, &av);
        if (ac && av)
        {
            long textAtomsSize = 0;
            char *textAtoms = NULL;
...
t_max_err miragebox_notify(t_miragebox *x, t_symbol *s, t_symbol *msg, void *sender, void *data)
{
    ...
    if (sender == x)
    {
        ...
        else if (msg == gensym("miragebox_valuechanged") || msg == gensym("miragebox_valuerequest"))
        {
    ...
    else if (msg == _sym_attr_modified)
    {
