...
void *Huygens_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if (x = (t_Huygens *)object_alloc((t_class*)Huygens_class)) 
	{
		...
		if(argv[0].a_type == A_LONG) x->f_outputNumber = atom_getlong(argv);
		if(x->f_outputNumber <= 0) x->f_outputNumber = 16;
		if(argv[1].a_type == A_LONG) x->f_mode = atom_getlong(argv+1);
		if(x->f_mode < 0 || x->f_mode > 1) x->f_mode = 0;
		if(argv[2].a_type == A_FLOAT)x->f_speakersDelta = (double)atom_getfloat(argv+2);
		if(x->f_speakersDelta <= 0.) x->f_speakersDelta = 0.189;
		if(argv[3].a_type == A_FLOAT)x->f_speakersDistance = (double)atom_getfloat(argv+3);
		if(x->f_speakersDistance <= 0.) x->f_speakersDistance = 0.5;
		if(argv[4].a_type == A_LONG)x->f_interpolation = (double)atom_getlong(argv+4);
		if(x->f_interpolation < 0 || x->f_interpolation > 2) x->f_interpolation = 2;
		...
		x->f_wfsCircular = new CircularWfs(x->f_outputNumber, x->f_speakersDelta, -1.10);
		x->f_wfsCircular->setReferenceDistance(x->f_speakersDistance);
		x->f_wfsCircular->setSpeakersDelta(x->f_speakersDelta);
		x->f_wfsCircular->setSamplingRate(x->f_sr);
...
//		else 
//		{
//			x->f_wfsCircular->setVirtualSourcePos(x->f_positionX, x->f_positionY);
//			tmp_output = x->f_wfsCircular->process(signals[0][i]);
//		}
//		
...
void Huygens_perform64(t_Huygens *x, t_object *dsp64, double **ins, long numins, double **outs, long numouts, long sampleframes, long flags, void *userparam)
{
	...
	for (i = 0; i < sampleframes; i++) 
	{
		...
		{
			x->f_wfsCircular->setVirtualSourcePos(x->f_positionX, x->f_positionY);
			tmp_output = x->f_wfsCircular->process(ins[0][i]);
		}
...
t_max_err distance_set(t_Huygens *x, t_object *attr, long argc, t_atom *argv)
{
	x->f_speakersDistance = (double)atom_getfloat(argv);
	if (x->f_wfsLinear)
	{
		x->f_wfsLinear->setReferenceDistance(x->f_speakersDistance);
	}
...
{
	x->f_speakersDistance = (double)atom_getfloat(argv);
	if (x->f_wfsLinear != NULL)
		x->f_wfsLinear->setSpeakersDelta(x->f_speakersDelta);
	if (x->f_wfsCircular != NULL)
		x->f_wfsCircular->setSpeakersDelta(x->f_speakersDelta);
	return 0;
}
